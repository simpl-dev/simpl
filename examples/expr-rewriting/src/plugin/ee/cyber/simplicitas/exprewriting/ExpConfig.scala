...
object ExpConfig {
  ...
  val instance = new ExpConfig()
  val languageId = "exp_rewriting"
  val language = LanguageRegistry.findLanguage(languageId)
  val pluginId = "exp_rewriting"
...
class ExpConfig extends APluginConfig {
  ...
  override def runGenerator(dir: String, file: String) {
    ExpMain.main(Array("--dest", dir, dir + file))
  }
}
